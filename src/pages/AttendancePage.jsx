...
export default function AttendancePage() {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease', paddingBottom: '72px' }}>
      {loading && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
        <div style={{ background: 'white', borderRadius: '12px', padding: '16px 20px', border: '1px solid #F3F4F6', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '28px' }}>🧑‍🎓</span>
            <div style={{ textAlign: 'right' }}>
              <p style={{ fontSize: '12px', color: '#9CA3AF' }}>إجمالي الطلاب</p>
              <p style={{ fontSize: '34px', fontWeight: 900, color: '#111827', lineHeight: 1 }}>{students.length}</p>
            </div>
          </div>
        </div>
        <div style={{ background: 'white', borderRadius: '12px', padding: '16px 20px', border: '1px solid #F3F4F6', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <p style={{ fontSize: '34px', fontWeight: 900, color: '#8B1A1A', lineHeight: 1 }}>
                %{rate}
              </p>
            </div>
          </div>
      ...
      >
        ...
        <button onClick={() => { const all = {}; filtered.forEach(s => { all[s.id] = 'حاضر'; }); setAttendance(p => ({...p, ...all})); }} style={{
        ...
        {visible.map(s => {
          ...
          return (
            <div key={s.id} style={{
              ...
              borderRight: status === 'حاضر' ? '3px solid #16A34A' : status === 'غائب' ? '3px solid #DC2626' : status === 'معتذر' ? '3px solid #D97706' : '3px solid transparent',
            }}>
              {/* Name */}
      ...
      {/* Sticky bottom bar */}
      <div style={{
        position: 'fixed', bottom: 0, left: 0, right: '240px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            style={{ padding: '6px 10px', borderRadius: '6px', border: '1px solid #E5E7EB', background: 'white', cursor: 'pointer', color: '#6B7280', fontFamily: 'Cairo', fontSize: '14px' }}>›</button>
          <span style={{ fontSize: '13px', fontWeight: 700, color: '#111827' }}>{page}/{totalPages}</span>
          <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
            style={{ padding: '6px 10px', borderRadius: '6px', border: '1px solid #E5E7EB', background: 'white', cursor: 'pointer', color: '#6B7280', fontFamily: 'Cairo', fontSize: '14px' }}>‹</button>
        </div>

        <p style={{ fontSize: '12px', color: '#9CA3AF' }}>{filtered.length} طالب من أصل {students.length}</p>

        {/* Save */}
        {saved ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '8px', background: '#DCFCE7', color: '#16A34A', fontSize: '13px', fontWeight: 700 }}>
          ...
          <button onClick={save} disabled={!marked || saving} style={{
            ...
            {saving ? 'جاري الحفظ...' : `حفظ الحضور (${marked}/${students.length})`}
          </button>
        )}
      </div>
      </>
