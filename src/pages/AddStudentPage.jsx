...
import {
  User, Home, Info, Save, UserPlus, X, ChevronDown,
  Phone, Heart, BookOpen, MapPin, AlertCircle,
} from 'lucide-react';
...
export default function AddStudentPage() {
  ...
  async function handleSave(addAnother = false) {
    ...
    try {
      ...
      // Show a visible Arabic error — do NOT navigate
      const msg = err?.message || 'حدث خطأ غير متوقع';
      setSubmitError(`تعذّر حفظ الطالب: ${msg}`);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } finally {
      setSaving(false);
    }
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      {saved && (
        <div style={{
          ...
          ✓ تم حفظ الطالب بنجاح! يمكنك إضافة طالب جديد.
        </div>
      )}
